...
from src.types import TripRequest, TripPurpose, TransportMethod
...
class TripCommands(commands.Cog):
    ...
    @app_commands.command(
        name="trip",
        description="旅行準備アシスタントのメインコマンド"
    )
    @app_commands.describe(
        subcommand="実行するサブコマンド (smart/check/help)"
    )
    async def trip(
        self,
    ...
    async def trip_smart(
        ...
        end_date: str,
        purpose: TripPurpose,
        transport: Optional[TransportMethod] = None
    ):
        """スマートチェックリストを生成."""
    ...
    def create_checklist_embed(self, checklist) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            color=discord.Color.green() if checklist.completion_percentage >= 80 else discord.Color.blue()
        )
