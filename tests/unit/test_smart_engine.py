...
class TestSmartTemplateEngine:
    """SmartTemplateEngineのテスト."""

    @pytest.fixture
    def engine(self) -> SmartTemplateEngine:
        """テスト用のSmartTemplateEngineインスタンス."""
        return SmartTemplateEngine()

    @pytest.fixture
    def sample_request(self) -> TripRequest:
        """テスト用のTripRequest."""
        return TripRequest(
            destination="札幌",
    ...
    def test_init(self, engine: SmartTemplateEngine):
        """初期化のテスト."""
    ...
    def test_select_template_sapporo_business(self, engine: SmartTemplateEngine):
        """札幌出張テンプレート選択."""
        request = TripRequest(
            destination="札幌",
            start_date=date(2025, 6, 28),
    ...
    def test_select_template_general_business(self, engine: SmartTemplateEngine):
        """一般出張テンプレート選択."""
        request = TripRequest(
            destination="東京",
    ...
    def test_select_template_leisure(self, engine: SmartTemplateEngine):
        """レジャーテンプレート選択."""
        request = TripRequest(
            destination="沖縄",
    ...
    def test_prepare_context(self, engine: SmartTemplateEngine, sample_request: TripRequest):
        """テンプレート用コンテキストの準備."""
    ...
    def test_prepare_context_long_trip(self, engine: SmartTemplateEngine):
        """長期旅行のコンテキスト準備."""
        request = TripRequest(
            destination="大阪",
            start_date=date(2025, 7, 1),
    ...
    def test_get_transport_display(self, engine: SmartTemplateEngine):
        """交通手段の表示名取得."""
    ...
    def test_normalize_category(self, engine: SmartTemplateEngine):
        """カテゴリ名の正規化."""
    ...
    def test_extract_items_from_markdown(self, engine: SmartTemplateEngine):
        """Markdownからのアイテム抽出."""
        # markdown_processorをモック
        engine.markdown_processor.extract_checklist_items = Mock(
            return_value=[
                ("移動関連", "パスポート", False),
                ("仕事関連", "ノートPC", False),
                ("ビジネス", "名刺", True),  # 正規化テスト
    ...
    def test_get_regional_adjustments_hokkaido_winter(self, engine: SmartTemplateEngine):
        """北海道冬季の地域調整."""
        request = TripRequest(
            destination="北海道札幌市",
    ...
    def test_get_regional_adjustments_hokkaido_summer(self, engine: SmartTemplateEngine):
        """北海道夏季の地域調整."""
        request = TripRequest(
            destination="札幌",
    ...
    def test_get_regional_adjustments_okinawa(self, engine: SmartTemplateEngine):
        """沖縄の地域調整."""
        request = TripRequest(
            destination="沖縄県那覇市",
    ...
    def test_get_duration_adjustments_long_trip(self, engine: SmartTemplateEngine):
        """長期滞在の調整."""
        request = TripRequest(
            destination="東京",
            start_date=date(2025, 7, 1),
    ...
    def test_get_duration_adjustments_short_trip(self, engine: SmartTemplateEngine):
        """短期滞在の調整."""
        request = TripRequest(
            destination="大阪",
    ...
    def test_get_transport_adjustments_airplane(self, engine: SmartTemplateEngine):
        """飛行機利用時の調整."""
        request = TripRequest(
            destination="福岡",
            start_date=date(2025, 8, 1),
    ...
    def test_get_transport_adjustments_car(self, engine: SmartTemplateEngine):
        """車利用時の調整."""
        request = TripRequest(
            destination="名古屋",
    ...
    async def test_apply_adjustments(
        self, engine: SmartTemplateEngine, sample_request: TripRequest
    ):
        """調整適用の統合テスト."""
        base_items = [
    ...
    async def test_generate_checklist_full_flow(
        ...
        # markdown_processorをモック
        mock_processor = Mock()
        mock_processor.combine_templates.return_value = """
# 札幌出張チェックリスト
...
- [ ] 名刺
"""
        mock_processor.extract_checklist_items.return_value = [
            ("移動関連", "パスポート", False),
            ("移動関連", "航空券", False),
            ("仕事関連", "ノートPC", False),
            ("仕事関連", "名刺", False),
        ...
        engine.markdown_processor = mock_processor
        ...
        mock_processor.combine_templates.assert_called_once()
        call_args = mock_processor.combine_templates.call_args
        assert call_args[0][0] == "base_travel.md"
        assert call_args[0][1] == "business.md"

    @pytest.mark.asyncio
    async def test_generate_checklist_leisure(self, engine: SmartTemplateEngine):
        """レジャー用チェックリスト生成."""
        request = TripRequest(
            destination="沖縄",
            start_date=date(2025, 8, 10),
        ...
        # markdown_processorをモック
        mock_processor = Mock()
        mock_processor.combine_templates.return_value = "# レジャーチェックリスト"
        mock_processor.extract_checklist_items.return_value = []
        engine.markdown_processor = mock_processor
        ...
        call_args = mock_processor.combine_templates.call_args
        assert call_args[0][0] == "base_travel.md"
        assert call_args[0][1] == "leisure.md"
