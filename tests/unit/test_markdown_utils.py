...
""",
    ...
    def test_from_file_invalid_format(self, tmp_path: Path):
        """不正な形式のファイルからの読み込み."""
        # Front Matterのないファイル
...
class TestMarkdownProcessor:
    ...
    @pytest.fixture
    def processor(self, tmp_path: Path) -> MarkdownProcessor:
    ...
    def test_init_default_template_dir(self):
        ...
        with patch("src.utils.markdown_utils.settings") as mock_settings:
            mock_settings.TEMPLATE_PATH = "/default/path"
            processor = MarkdownProcessor()
            assert processor.template_dir == Path("/default/path")

    def test_load_template_success(self, processor: MarkdownProcessor, create_test_template):
        """テンプレートの正常な読み込み."""
...
""",
    ...
    def test_load_template_not_found(self, processor: MarkdownProcessor):
        """存在しないテンプレートの読み込み."""
        with pytest.raises(TemplateNotFoundError, match="テンプレートが見つかりません"):
            processor.load_template("non_existent.md")

    def test_render_template_basic(self, processor: MarkdownProcessor, create_test_template):
        """基本的なテンプレートレンダリング."""
        create_test_template(
            "basic.md",
...
""",
    ...
    def test_render_template_with_defaults(
        self, processor: MarkdownProcessor, create_test_template
    ):
        """デフォルト値を使用したレンダリング."""
        create_test_template(
            "defaults.md",
...
""",
    ...
    def test_render_template_error(self, processor: MarkdownProcessor):
        """テンプレートレンダリングエラー."""
        with pytest.raises(TemplateNotFoundError, match="テンプレートレンダリングエラー"):
            processor.render_template("non_existent.md", {})

    def test_combine_templates(self, processor: MarkdownProcessor, create_test_template):
        """複数テンプレートの組み合わせ."""
        # ベーステンプレート
        create_test_template(
            "base.md",
...
""",
    ...
    def test_combine_templates_missing_module(
        self, processor: MarkdownProcessor, create_test_template
    ):
        """存在しないモジュールを含む組み合わせ."""
    ...
    def test_extract_checklist_items_basic(self, processor: MarkdownProcessor):
        """基本的なチェックリスト項目の抽出."""
        markdown = """# チェックリスト
...
"""
    ...
    def test_extract_checklist_items_with_icons(self, processor: MarkdownProcessor):
        """アイコン付きカテゴリからの抽出."""
        markdown = """## 🎫 移動関連
- [ ] チケット
...
"""
    ...
    def test_extract_checklist_items_empty(self, processor: MarkdownProcessor):
        """チェックリスト項目がない場合."""
        markdown = """# ただのドキュメント
...
"""
    ...
    def test_update_checklist_status(self, processor: MarkdownProcessor):
        """チェックリスト状態の更新."""
        original = """## カテゴリ
- [ ] 項目1
- [x] 項目2
...
"""
    ...
    def test_update_checklist_status_preserve_indent(self, processor: MarkdownProcessor):
        """インデントを保持した更新."""
        original = """## カテゴリ
  - [ ] インデント項目
    - [ ] ネスト項目
"""
    ...
    def test_update_checklist_status_no_match(self, processor: MarkdownProcessor):
        """マッチしない項目の更新."""
        original = """- [ ] 項目A
- [ ] 項目B
"""
